...
function ProductList() {
  ...
  const filteredPlantsArray = plantsArray.filter(
    (category) =>
      (!selectedCategory || category.category === selectedCategory) &&
      category.plants.some((plant) =>
  ...
  const handleAddToCart = (plant) => {
    dispatch(
      addItem({
        name: plant.name,
        cost: parseFloat(plant.cost.slice(1)),
        image: plant.image,
      })
    );
    setNotification(`${plant.name} has been added to the cart!`);
    setTimeout(() => setNotification(""), 3000);
  };

  const handleContinueShopping = () => {
    // Switch view to shopping (products)
    setShowCart(false);
  };
